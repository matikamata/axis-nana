#!/usr/bin/env python3
"""
AXIS-NIDDHI — oracle_engine.py
AXIS Oracle — Core Reasoning Engine V1.0

ARCHITECTURE:
  Modality-agnostic. Returns structured OracleResponse.
...
INVARIANT:
  Oracle never synthesizes knowledge. It retrieves and cites.
...
class OracleResponse:
    """
    Modality-agnostic oracle output.
    ...
    def to_graph_hint(self) -> dict:
        """Node/edge output for graph rendering (Cosmos)."""
        nodes = [{"id": c.slug, "label": c.pali, "type": c.type_}
                 for c in self.concepts]
        edges = []
        for c in self.concepts:
            for r in c.related:
                edges.append({"from": c.slug, "to": r, "weight": 1})
        return {
            "nodes":     nodes,
            "edges":     edges,
...
class OracleEngine:
    """
    AXIS Oracle — modality-agnostic reasoning engine.
    Produces OracleResponse from natural language queries.
    """

    def __init__(self):
        self.kb = OracleKnowledgeBase()

    def ask(self, question: str, mode: str = "qa") -> OracleResponse:
        """Main entry point. mode: qa | explain | quiz | study_path | cite"""
    ...
    def _build_prompt(self, question: str,
        ...
        return f"""You are AXIS Oracle — a canonical question-answering engine for the PureDhamma corpus.
...
def ask(question: str, mode: str = "qa") -> OracleResponse:
